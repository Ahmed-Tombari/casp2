...
export async function generateMetadata({ params }: { params: Promise<{ pages: string }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale: page, namespace: 'quizzes.handwritingExercises' });
  return { title: t('title'), description: t('description') };
}

export default async function HandwritingExercisesPage({ params }: { params: Promise<{ pages: string }> }) {
  const { pages: page } = await params;
  const t = await getTranslations({ locale: page, namespace: 'quizzes.handwritingExercises' });
