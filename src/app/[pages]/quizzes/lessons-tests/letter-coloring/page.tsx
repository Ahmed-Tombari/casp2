...
export async function generateMetadata({ params }: { params: Promise<{ pages: string }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale: page, namespace: 'quizzes.letterColoring' });
  return { title: t('title'), description: t('description') };
}

export default async function LetterColoringPage({ params }: { params: Promise<{ pages: string }> }) {
  const { pages: page } = await params;
  const t = await getTranslations({ locale: page, namespace: 'quizzes.letterColoring' });
           ...
           <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {pages.map((page, i) => (
                 <div key={i} className="group relative rounded-[2rem] overflow-hidden border-4 border-gray-100 dark:border-gray-800 hover:border-brand-sky transition-all duration-300">
                    ...
                    {/* Preview Area */}
                    <div className={`aspect-square ${page.color} flex flex-col items-center justify-center relative`}>
                        <span className="text-[8rem] font-bold opacity-20 group-hover:scale-110 transition-transform duration-500">{page.letter}</span>
                        <div className="absolute bottom-4 font-bold uppercase tracking-widest opacity-60">{page.animal}</div>
                    ...
                    <div className="p-4 bg-white dark:bg-brand-navy-dark text-center">
                       <h3 className="font-bold text-gray-700 dark:text-gray-200">{t('letter')} {page.letter}</h3>
                    </div>
                 </div>
              ))}
           </div>
