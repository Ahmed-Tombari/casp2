...
export async function generateMetadata({ params }: { params: Promise<{ pages: string }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale: page, namespace: 'services.progress' });
  return { title: t('title') };
}

export default async function ProgressPage({ params }: { params: Promise<{ pages: string }> }) {
  const { pages: page } = await params;
  const t = await getTranslations({ locale: page, namespace: 'services.progress' });
  ...
  return (
    // FIX: Explicitly using dark:bg-[#09121E] for the page background
