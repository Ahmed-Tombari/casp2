...
export async function generateMetadata({ params }: { params: Promise<{ pages: string }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale: page, namespace: 'services.account' });
  return { title: t('title') };
}

export default async function AccountPage({ params }: { params: Promise<{ pages: string }> }) {
  const { pages: page } = await params;
  const t = await getTranslations({ locale: page, namespace: 'services.account' });
  ...
  return (
    ...
    <main className="min-h-screen bg-gray-50 dark:bg-[#09121E] pt-32 pb-20 transition-colors duration-300">
      <div className="container mx-auto max-w-4xl px-4">
        ...
        {/* FIX: Card background set to dark:bg-[#112240] (Lighter Navy) for contrast against the page */}
