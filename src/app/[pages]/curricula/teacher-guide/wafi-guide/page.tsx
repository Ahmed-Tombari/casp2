...
export async function generateMetadata({ params }: { params: Promise<{ pages: string }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale: page, namespace: 'teacherGuide.wafiGuide' });
  return { title: t('title'), description: t('description') };
}

export default async function WafiGuidePage({ params }: { params: Promise<{ pages: string }> }) {
  const { pages: page } = await params;
  const t = await getTranslations({ locale: page, namespace: 'teacherGuide.wafiGuide' });
  const common = await getTranslations({ locale: page, namespace: 'teacherGuide' });

  const files = [
    { title: t('vol1Guide'), size: '12.5 MB' },
    { title: t('vol2Guide'), size: '14.2 MB' },
    { title: t('vol3Guide'), size: '11.8 MB' },
    { title: t('vol4Guide'), size: '13.0 MB' },
  ];

  return (
    <main className="min-h-screen bg-gray-50 dark:bg-background">
      <GuideHero title={t('title')} desc={t('description')} icon="solar:library-bold-duotone" />
