...
export async function generateMetadata({ params }: { params: Promise<{ pages: string }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale: page, namespace: 'teacherGuide' });

  return {
    title: t('title'),
    description: t('description'),
    alternates: {
      canonical: `/${page}/teacher-guide`,
      languages: {
        'ar': '/ar/teacher-guide',
        'fr': '/fr/teacher-guide',
        'en': '/en/teacher-guide',
...
export default async function TeacherGuidePage({ params }: { params: Promise<{ pages: string }> }) {
  const { pages: page } = await params;
  const t = await getTranslations({ locale: page, namespace: 'teacherGuide' });
  const isRTL = page === 'ar';
