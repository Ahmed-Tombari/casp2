...
export async function generateMetadata({ params }: { params: Promise<{ pages: string }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale: page, namespace: 'teacherGuide.mufidGuide' });
  return { title: t('title'), description: t('description') };
}

export default async function MufidGuidePage({ params }: { params: Promise<{ pages: string }> }) {
  const { pages: page } = await params;
  const t = await getTranslations({ locale: page, namespace: 'teacherGuide.mufidGuide' });
  const common = await getTranslations({ locale: page, namespace: 'teacherGuide' });

  const files = [
    { title: t('readingGuide'), size: '3.5 MB' },
    { title: t('writingGuide'), size: '4.1 MB' },
    { title: t('speakingGuide'), size: '3.2 MB' },
  ];

  return (
    <main className="min-h-screen bg-gray-50 dark:bg-background">
      <GuideHero title={t('title')} desc={t('description')} icon="solar:notebook-bold-duotone" />
