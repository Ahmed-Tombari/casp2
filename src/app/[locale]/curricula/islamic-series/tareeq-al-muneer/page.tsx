...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale: locale, namespace: 'islamicSeries.tareeqAlMuneerAr' });

  return {
    title: t('title'),
    description: t('description'),
    alternates: {
      canonical: `/${locale}/curricula/tareeq-al-muneer`,
      languages: {
        ar: '/ar/curricula/tareeq-al-muneer',
        fr: '/fr/curricula/tareeq-al-muneer',
        en: '/en/curricula/tareeq-al-muneer',
      },
    },
  };
}
            ...
            {milestones.map((level) => (
              ...
              >
                ...
                <div className="text-center sm:rtl:text-right sm:ltr:text-left z-10">
                   ...
                   <Link href={`/curricula/tareeq-al-muneer/level-${level.id}`} className="inline-flex items-center gap-2 text-brand-navy font-bold hover:gap-3 transition-all text-sm uppercase tracking-wider">
                      <span>{t('explore')}</span>
